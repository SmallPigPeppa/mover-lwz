...
class OpenPose_Pose2Room(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # add check useful.txt
        # useful_list_fn = '/is/cluster/scratch/scene_generation/Pose2Room_output/useful.txt'
        # with open(useful_list_fn) as fin:
        #     useful_list = fin.readlines()
        #     useful_list = [one[:-6] for one in useful_list]

        assert len(idx) == 1
        img_path = self.img_paths[idx[0]]
        img_name = os.path.basename(img_path).split('.')[0]

        # print(f'img_path: {img_name}, use_list: {useful_list[0]}')
        # if img_name in useful_list:
        #     print(f'exist {img_name}')
        #     return None
        # else:
        #     return self.read_item(img_path)
        return self.read_item(img_path)
    ...
    def read_item(self, sample_file):
        ...
        # skeleton_joints_debug = np.tile(skeleton_joints_sample6[np.newaxis, :, :], (skeleton_joints.shape[0], 1, 1))

        # face and contour
        keyp_tuple = read_keypoints_VH(skeleton_joints, use_hands=self.use_hands,
                                       use_face=self.use_face,
                                       use_face_contour=self.use_face_contour)
        # keyp_tuple = read_keypoints_VH(skeleton_joints_debug, use_hands=self.use_hands,
        #                                use_face=self.use_face,
        #                                use_face_contour=self.use_face_contour)
        if self.single:
            batch_size = 1
        ...
        # img_fn_tmp = glob.glob('/mnt/mmtech01/dataset/vision_text_pretrain/gta-im/FPS-5/2020-06-11-10-06-48/*.png')
        # img_fn_tmp = [img for img in img_fn_tmp if not img.endswith('_id.png')]
        img_fn=[]
        for i in img_fn_tmp:
            # if int(i.split('/')[-1].split('.')[0]) in range(408,955):
            if int(i.split('/')[-1].split('.')[0]) in range(0, 955):
                img_fn.append(i)
