...
class SMPLifyLoss3D(single_view_fitting.SMPLifyLoss):
    ...
    def forward(self, body_model_output, gt_joints,
        ...
        if gt_joints.shape[-1] == 3:
            ...
            print('body_model_output.full_pose.shape:', body_model_output.full_pose.shape)
...
class SMPLifyLoss3D_withHands(single_view_fitting.SMPLifyLoss):
    def __init__(self, search_tree=None,
        ...
        super(SMPLifyLoss3D_withHands, self).__init__(search_tree=search_tree,
                                                      ...
                                                      tri_filtering_module=tri_filtering_module,
                                                      rho=rho,
                                                      body_pose_prior=body_pose_prior,
                                                      shape_prior=shape_prior,
                                                      angle_prior=angle_prior,
...
class MultiViewSMPLifyLoss(single_view_fitting.SMPLifyLoss):
    def __init__(self, **kwargs):
        ...
        if self.ordinal_depth:
            self.register_buffer('ordinal_depth_loss_weight',
                                 torch.tensor(kwargs['ordinal_depth_loss_weight'], dtype=kwargs['dtype']))
        ...
        for vi, cam in enumerate(cameras):
            ...
            if self.video_smooth:
                if self.motion_smooth_prior_flag:
                    ...
                    motion_prior_loss = self.motion_smooth_prior(model_verts, model_joints,
                                                                 conf=motion_useful_conf)  # per-single image loss.
                ...
                if self.constant_velocity:
                    ...
                    if True:
                        ...
                        joints_2d_diff = self.robustifier(
                            projected_joints[2:] + projected_joints[:-2] - 2 * projected_joints[1:-1])
                        ...
                        model_joints = body_model_output.joints
                        joints_3d_diff = self.robustifier_3DJoint(
                            model_joints[2:] + model_joints[:-2] - 2 * model_joints[1:-1])
        ...
        if True:  # old version for single image contact loss.
            ...
            if self.ground_contact_support and self.gp_contact_loss_weight > 0:
                ...
                if body_vertices_contact_label_feet.sum() > 0:
                    gp_contact_loss = dist_gp[body_vertices_contact_label_feet.squeeze(
                        -1)].mean() * self.gp_contact_loss_weight
                else:
            ...
            if self.ground_contact_support and self.gp_contact_loss_weight > 0:
                ...
                tmp_gp_contact_dist = F.mse_loss(feet_vertices, gt_ground_plane.expand_as(feet_vertices),
                                                 reduction='none').sqrt()
                ...
                l1_gp_dist = torch.abs(
                    torch.max(feet_vertices.reshape(batch_size, -1), -1)[0] - gt_ground_plane).unsqueeze(1).expand(-1,
                                                                                                                   2)
            ...
            if False:
                ...
                if self.contact and self.contact_loss_weight > 0:
                    ...
                    for idx in range(batch_size):
                        # if idx == 7:
                        #     import pdb;pdb.set_trace()
                        tmp_contact = scene_model(body_vertices_world[idx:idx + 1], stage=2,
                                                  contact_verts_ids=self.contact_verts_ids,
                                                  contact_angle=self.contact_angle,
                                                  contact_robustifier=self.contact_robustifier, ftov=self.ftov)
                ...
                if self.ordinal_depth and self.ordinal_depth_loss_weight > 0:
                    ...
                    for idx in range(batch_size):
                        ...
                        # this 
                        tmp_depth_loss = \
                        scene_model.compute_ordinal_depth_loss_perframe(body_vertices_world[idx:idx + 1], idx,
                                                                        use_for_human=self.use_human_depth)[
                            'loss_depth']
                        # import pdb;pdb.set_trace()
                        if torch.isnan(
                                tmp_depth_loss):  # the depth loss is too big, generates large gradient, leads to error !!!
                            import pdb;
                            pdb.set_trace()
                ...
                scene_loss_dict = scene_model(body_vertices_world,
                                              ...
                                              contact_angle=self.contact_angle, \
                                              contact_robustifier=self.gp_contact_robustifier,
                                              ftov=self.ftov)  # self.contact_robustifier
