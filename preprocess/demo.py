...
sys.path.append('merge_preprocess')
...
def main(args):
    ...
    if demo_mode == 'video':
        ...
        if os.path.isdir(os.path.join(output_path, 'tmp_images')):
            ...
            if os.path.exists(os.path.join(input_image_folder, '000001.png')):
                img_shape = cv2.imread(os.path.join(input_image_folder, '000001.png')).shape
            elif os.path.exists(os.path.join(input_image_folder, '000001.jpg')):
                img_shape = cv2.imread(os.path.join(input_image_folder, '000001.jpg')).shape
            else:
                exit(f'error when load images.')
        else:
    ...
    if args.mode == 'video':
        ...
        total_time = time.time()
        detection_results = tester.run_detector(input_image_folder)
        pare_time = time.time()
        pare_results = tester.run_on_image_folder(input_image_folder, detection_results)
        end = time.time()
