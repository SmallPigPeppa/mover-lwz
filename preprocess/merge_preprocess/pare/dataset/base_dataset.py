...
class BaseDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # TODO remove ochuman, crowdpose exception
        if not self.is_train: # or self.dataset in ['ochuman', 'crowdpose']:
            # if self.dataset in ['ochuman', 'crowdpose']:
            #     if time.localtime().tm_mday > 28 and time.localtime().tm_mday >=10:
            #         raise ValueError('remove ochuman, crowdpose exception from the base dataset!!!')
        ...
        if not self.is_train:
            if self.dataset in ['3dpw', '3dpw-all', '3doh']:
        ...
        #         raise ValueError('remove ochuman, crowdpose exception from the base dataset!!!')
        #     try:
        #         item['sample_idx'] = self.data['indices'][index]
        #     except:
        #         raise ValueError(f'there is not a sample idx for this sample {index}')
