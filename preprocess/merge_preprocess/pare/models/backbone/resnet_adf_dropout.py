...
class Bottleneck(nn.Module):
    ...
    def forward(self, inputs_mean, inputs_variance):
        ...
        out_mean, out_var = self.bn3(*self.conv3(*out))
        shortcut_mean, shortcut_var = self.shortcut(*x)
        out = out_mean + shortcut_mean, out_var + shortcut_var
        out = self.dropout(*self.ReLU(*out))
        return out
...
class ResNetADFDropout(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, p=0.2, noise_variance=1e-3, min_variance=1e-3, initialize_msra=False):
        ...
        self.conv1 = adf.Conv2d(3, 64, kernel_size=7, stride=2, padding=3, bias=False, keep_variance_fn=self.keep_variance_fn)
        self.bn1 = adf.BatchNorm2d(64, keep_variance_fn=self.keep_variance_fn)
        self.ReLU = adf.ReLU(keep_variance_fn=self.keep_variance_fn)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        # self.maxpool = adf.MaxPool2d(keep_variance_fn=self.keep_variance_fn) # nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
    ...
    def forward(self, x):
        ...
        out = self.maxpool(self.ReLU(*self.bn1(*self.conv1(*x)))[0])
        out = out, torch.zeros_like(out) + self._noise_variance
...
def resnet18_adf_dropout(p=0.2, noise_variance=1e-3, min_variance=1e-3):
...
def resnet34_adf_dropout(p=0.2, noise_variance=1e-3, min_variance=1e-3):
    return ResNetADFDropout(BasicBlock, 
                            [3,4,6,3],  
                            num_classes=10, 
                            p=p,
...
def resnet50_adf_dropout(p=0.2, noise_variance=1e-3, min_variance=1e-3):
...
def resnet101_adf_dropout(p=0.2, noise_variance=1e-3, min_variance=1e-3):
    return ResNetADFDropout(Bottleneck, 
                            [3,4,23,3],  
                            num_classes=10, 
                            p=p,
...
def resnet152_adf_dropout(p=0.2, noise_variance=1e-3, min_variance=1e-3):
    return ResNetADFDropout(Bottleneck, 
                            [3,8,36,3], 
                            num_classes=10, 
                            p=p,
