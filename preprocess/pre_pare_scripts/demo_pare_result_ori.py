...
def main(**args):
    ...
    if not visualize:
        ...
        # preprocess_mv_input(img_folder, json_folder, output_folder+'_PARE3DJoint_OP2DJoints')
    ...
    for idx in tqdm(range(len(pare_result_dict.frame_ids))):
        ...
        # TODO: add rotation to the body pose
        # import pdb;pdb.set_trace()
        ...
        final_rot_joints3d = rot_joints3d + trans
        ...
        if visualize:
            ...
            tmp_rot = np.matmul(GLOBAL_CAM_EXT, rot_joints3d.T).T
            proj_3d = np.matmul(GLOBAL_CAM_INC, (tmp_rot + trans).T).T
        ...
        if export_mesh:
            os.makedirs(os.path.join(output_folder, 'meshes'), exist_ok=True)
            mesh_fn = os.path.join(output_folder, 'meshes' , f'{frame_id:06d}.obj')
        ...
        if save_new_json:
            ...
            with open(json_fn, 'r') as fin:
                ...
                if pare_right_angle.size > 0 and pare_left_angle.size > 0 and \
                    len(right_angle) > 0 and len(left_angle) > 0 and \
                    np.linalg.norm(pare_right_angle-right_angle[:-1]) > 100 and \
                    np.linalg.norm(pare_read_keypointsight_angle-right_angle[:-1]) < 300 and \
                    np.linalg.norm(pare_right_angle-right_angle[:-1]) - 4 * np.linalg.norm(pare_right_angle-left_angle[:-1]) > 0:
                    print(f'inverse feet in {idx}, replace angle and feet in a reverse way')
                    print(f'Ori: {np.linalg.norm(pare_right_angle-right_angle[:-1]) }, New: {np.linalg.norm(pare_right_angle-left_angle[:-1])}')
                    result['people'][0]['pose_keypoints_2d'][3*11:3*12] = left_angle
